...
class ContentBlock(BaseModel):
    """内容块基类"""
    type: str = Field(..., description="内容块类型")
...
class MessageRequest(BaseModel):
    ...
    temperature: float = Field(default=1.0, ge=0.0, description="采样温度")
...
class Usage(BaseModel):
    ...
    cache_creation_input_tokens: Optional[int] = Field(default=0, description="缓存创建输入token")
    cache_read_input_tokens: Optional[int] = Field(default=0, description="缓存读取输入token")
...
def format_anthropic_messages(messages: List[Message], system: Optional[str] = None) -> str:
...
async def anthropic_stream_generator(
    ...
    delta_event = {
        ...
        "usage": {
            "output_tokens": len(content.split())  # 简化计算
        }
    }
    yield json.dumps(delta_event) + "\n"
...
async def create_message(request: MessageRequest):
    ...
    try:
        ...
        top_k_val = request.top_k if request.top_k is not None else 0  # 必须是0，不能是None！
        
        sampler = make_sampler(
            temp=temp_val if temp_val > 0 else 0.0,
            top_p=top_p_val if top_p_val > 0 else 1.0,
            top_k=top_k_val if top_k_val and top_k_val > 0 else 0  # 必须是0，不能是None！
        )
...
async def create_message_stream(request: MessageRequest):
    ...
    try:
        ...
        return StreamingResponse(
            anthropic_stream_generator(response, request_id, request.model, request.max_tokens),
...
async def get_model(model_id: str):
    """获取指定模型信息"""
    available_models = ["qwen3-4b", "yuntu-llm-2b"]
